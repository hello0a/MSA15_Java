...
// 타입 매개변수를 A 클래스와 그 하위 클래스만 가능하도록 제한
class D <T extends A> {
...
//		D<X> d0 = new D<X>();
		...
		d1.setT(new B()); // B -> A 업캐스팅
		d1.setT(new C()); // C -> A 업캐스팅
...
//		d2.setT(new A());
		...
		// E <T extends Number>
		// Number 클래스로 제한해줘서 숫자 타입인 클래스만 타입지정 가능
		E<Integer> e1= new E<Integer>();
		E<Double> e2 = new E<Double>();
		E<Byte> e3 = new E<Byte>();
//		E<Boolean> e4 = new E<Boolean>(); --> 숫자 타입X
		...
		// e3 객체에 지정한 타입은 Byte(byte) -> 1byte 이기 때문에 강제 형변환 필요
		e3.setT((byte) 127);
